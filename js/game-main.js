...
alert('🎮 EVENTO match_started RECIBIDO');
  
  console.log('🎮 match_started recibido:', event.data);
  const data = JSON.parse(event.data);
  
  console.log('🔄 Cambiando a fase PLAY');
  GAME_STATE.phase = 'PLAY';
  GAME_STATE.turn = 'RED';

    alert('✅ Fase cambiada a PLAY'); 
  
  // RE-RENDERIZAR para activar draggable
  console.log('🎨 Re-renderizando tablero');
  renderer.render(board.matrix);
  
  statusDisplay = document.getElementById('game-status');
  statusDisplay.innerText = data.yourTurn ? 'TU TURNO' : 'turno del oponente';
...
function handleCombatResult(event) {
  ...
  console.log('🔍 attacker.team:', attacker.team, '| myTeam:', myTeam);
  
  const wasMyAttack = attacker.team === myTeam;

  console.log('🔍 wasMyAttack:', wasMyAttack);
  
  if (winner === 'ATTACKER') {
    board.matrix[defender.position.y][defender.position.x] = new Piece(attacker.rank, attacker.team);
