...
// cache local de usuarios para búsqueda en cliente
let usersCache = [];
// HTML original del login para poder restaurarlo
...
document.addEventListener('DOMContentLoaded', () => {
    // guardar el HTML original del login para restaurarlo después del registro
...
async function registerUser(username) {
    ...
    try {
        ...
        // si la respuesta no es JSON y la petición falló, lanzar error genérico
        if (!response.ok) throw new Error(`Error ${response.status} al conectar con el servidor`);
        throw new Error('Respuesta inválida del servidor');
    }

    if (!response.ok) {
        const msg = data && (data.message || data.error) ? (data.message || data.error) : `Error ${response.status}`;
        throw new Error(msg);
    }

    // aceptar distintas formas de respuesta: userId | id | sessionId
    currentUserId = data.userId || data.id || data.sessionId || null;
...
function switchToLobbyScreen() {
    ...
    // ocultar panel derecho para que el lobby use todo el ancho
    const right = document.querySelector('.right-panel');
    if (right) right.style.display = 'none';

    // expandir el contenedor grid para que la columna izquierda ocupe todo el ancho
...
function initializeLobbyControls() {
    ...
    // buscador de usuarios (búsqueda en tiempo real)
    ...
    if (userSearch) {
        ...
        userSearch.addEventListener('input', debounced);
    }
}
...
function debounce(fn, wait) {
    ...
    return function (evt) {
        clearTimeout(t);
        t = setTimeout(() => fn(evt), wait);
    };
}

/**
 * Filtra la cache de usuarios y renderiza resultados
 * @param {string} query
 */
